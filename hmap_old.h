...
#define hh_hmapinsert(map, key, sz, val) (HH__hmapinsert((void**) &(map), (key), (sz), (val), sizeof(*(val))))
#define hh_hmaplen(map)                  (((map) == NULL) ? 0 : hh_hmapheader(map)->len)
#define hh_hmapfree(map)                 (HH__hmapfree((void**) &(map)))

void
hh_hmapclear(void* map);
size_t
...
HH__hmapconfig(void** map_ptr, size_t val_size, hh_hmap_opt opt) {
    ...
    HH__darrgrow((void**) &map_hdr->bucket_lookup, 0, sizeof(size_t));
...
hh_hmapclear(void* map) {
    ...
    // clear values
    map_hdr->len = 0;
    // clear values from lookup array
    hh_darrclear(map_hdr->bucket_lookup);
    // clear key entries
    for(size_t bucket_index = 0; bucket_index < map_hdr->bucket_count; ++bucket_index) {
        hh_darrclear(map_hdr->buckets[bucket_index]);
    }
}
...
HH__hmapinsert(void** map_ptr, void* key, size_t key_size, void* val, size_t val_size) {
    ...
    char* bucket_start = map_hdr->buckets[bucket_index];
    HH_ASSERT(bucket_start < entry_end);
    hh_darrput(map_hdr->bucket_lookup, (size_t) (entry_end - bucket_start));
    return val_start;
}
...
hh_hmapremove(void* map, const void* key, size_t key_size) {
    ...
    do {
        if(map_hdr->key_size == 0) {
            ...
            entry_start += sizeof(size_t); // after the key_size size_t
        }  
        if(HH__hmap_comp(map, key, key_size, entry_start, entry_key_size) == 0) {
            if(map_hdr->key_size == 0) {
                entry_start -= sizeof(size_t); // now back at beginning of entry
                entry_size = sizeof(size_t) + entry_key_size + sizeof(size_t);
                entry_index = *((size_t*) (entry_start + sizeof(size_t) + entry_key_size)); // correct
            } else {
    ...
    return NULL;
found:
    HH_DBG("entry_key_size: %zu, entry_size: %zu, entry_index: %zu", entry_key_size, entry_size, entry_index);
    (void) NULL;
    ...
    if(entry_end < bucket_end) {
        ...
        while(ptr < bucket_end - entry_size) {
            ...
            ptr += ptr_key_size;
            map_hdr->bucket_lookup[*((size_t*) ptr)] = (size_t) (ptr - bucket_start);
            ptr += sizeof(size_t);
        }
    }
    ...
    *((size_t*) (bucket_start + map_hdr->bucket_lookup[entry_index])) = entry_index;
